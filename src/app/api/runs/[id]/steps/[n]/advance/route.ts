...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string; n: string }> }) {
  ...
  const respond = async () => NextResponse.json({ step: (await getSteps(db, runId))[stepIndex], run: await getRun(db, runId) });
  // 领域函数统一抛 RunError(内含 HTTP 状态);其他异常按 500 rethrow。fn 可能异步,guard 需 await
  const guard = async (fn: () => unknown | Promise<unknown>) => {
    try { await fn(); return await respond(); }
  ...
  if (step.executorType === "llm") {
    ...
    if (action === "retry" && step.status === "failed") return guard(async () => retryStep(db, runId, stepIndex));
    if (action === "manual_override" && ["failed", "pending"].includes(step.status)) return guard(async () => manualOverrideStep(db, runId, stepIndex, String(body.output ?? "")));
  }
  if (step.executorType === "manual" && action === "submit" && step.status === "pending") {
    db.update(stepRunsTable).set({ status: "done", output: String(body.output ?? ""), finishedAt: new Date().toISOString() }).where(and(eq(stepRunsTable.runId, runId), eq(stepRunsTable.stepIndex, stepIndex)));
    await syncRunStatus(db, runId);
    return respond();
  }
  if (step.executorType === "checkpoint") {
    if (action === "approve" && step.status === "pending") return guard(async () => approveCheckpoint(db, runId, stepIndex));
    if (action === "reject" && step.status === "pending") {
      return guard(async () => rejectCheckpoint(db, runId, stepIndex, String(body.note ?? ""), typeof body.target_index === "number" ? body.target_index : undefined));
    }
  }
  if (step.executorType === "script") {
    const exOf = async () => (def.executor_id ? ((await db.select().from(executorsTable).where(eq(executorsTable.id, def.executor_id)))[0] ?? null) : null);
    if (action === "execute" && step.status === "pending") {
      const ex = await exOf();
      if (!ex || ex.type !== "script" || !ex.enabled) return NextResponse.json({ error: "script 执行器不存在或未启用" }, { status: 409 });
      const task = (await db.select().from(tasksTable).where(eq(tasksTable.id, run.taskId)))[0];
      ...
      if (confirmRequired(def.command, ex)) {
        await db.update(stepRunsTable).set({ status: "awaiting_confirmation", input: command, startedAt: new Date().toISOString() }).where(and(eq(stepRunsTable.runId, runId), eq(stepRunsTable.stepIndex, stepIndex)));
        await syncRunStatus(db, runId);
        return NextResponse.json({ step: (await getSteps(db, runId))[stepIndex], command, risks: scanRisk(command), awaiting: true });
      }
    ...
    if (action === "retry" && step.status === "failed") return guard(async () => retryStep(db, runId, stepIndex));
    if (action === "manual_override" && step.status === "failed") return guard(async () => manualOverrideStep(db, runId, stepIndex, String(body.output ?? "")));
  }
  if (action === "skip" && step.status === "pending") return guard(async () => skipStep(db, runId, stepIndex));
  ...
  async function runScriptAndRespond(command: string, ex: { shell: string | null; timeoutMs: number }, workingDir: string) {
    ...
    await db.update(stepRunsTable).set({ status: "running", input: command, startedAt: new Date().toISOString() }).where(and(eq(stepRunsTable.runId, runId), eq(stepRunsTable.stepIndex, stepIndex)));
    await syncRunStatus(db, runId);
    ...
    await persistStepTerminal(db, runId, stepIndex, {
