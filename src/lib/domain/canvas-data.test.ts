...
describe("collectWidgetData", () => {
  it("空库(种子后)请求全部 9 类:各段为空态不抛错", async () => {
    ...
    const bundle = await collectWidgetData(db, ["counters", "todo", "calendar", "notes", "links", "goals", "vault", "radar", "quickactions"], "Asia/Shanghai", NOW);
  ...
  it("计数与分组正确:新增任务/笔记/链接后 counters 与 links.groups 反映", async () => {
    ...
    const bundle = await collectWidgetData(db, ["counters", "links"], "", NOW);
  ...
  it("todo 段:今日完成的任务保留在清单末尾(可取消勾选),逾期只列未完成", async () => {
    const db = createTestDb();
    seedIfEmpty(db);
    addTask(db, { title: "已完成的今日事", status: "done", dueDate: "2026-09-12" });
    const bundle = await collectWidgetData(db, ["todo"], "Asia/Shanghai", NOW);
  ...
  it("links 段限量:分类数与每类条数截断,total 仍返回全量(大书签库不撑爆首页)", async () => {    const db = createTestDb();
    seedIfEmpty(db);
    db.delete(links).run();
    const n = now();
    ...
    const bundle = await collectWidgetData(db, ["links"], "", NOW);
